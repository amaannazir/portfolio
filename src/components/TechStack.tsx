...
const TechStack = () => (
  <section id="skills" className="scroll-mt-20 py-16 sm:py-24">
    ...
    <div className="section-shell">
      <div className="mb-8 max-w-3xl sm:mb-10"><p className="eyebrow mb-3">Engineering stack</p><h2 className="font-display text-3xl font-medium sm:text-5xl">A focused toolkit for complete delivery.</h2><p className="mt-4 leading-relaxed text-muted-foreground sm:mt-5 sm:text-lg">One practical stack, organised around how systems are actually designed, integrated, shipped and supported.</p></div>
      <div className="grid border-l border-t border-border md:grid-cols-2">
        {groups.map((group) => { const Icon = group.icon; return <article key={group.title} className="border-b border-r border-border p-6 sm:p-8">
          ...
          <h3 className="mt-7 font-display text-2xl font-semibold sm:mt-10">{group.title}</h3><p className="mt-2 max-w-md text-sm leading-relaxed text-muted-foreground">{group.description}</p>
